...
A demand and its silence is evidence rather than proof, and notarial protest proved only presentment and refusal, never ability to pay. In the shielded pool, a holding proof binds the demand to real notes; it does not identify the demander or prove non-payment outside the record. A fresh key supplies no continuing reputation, even if it signs, so identity-based restraint on false allegations needs separate evidence and is not supplied by the protocol. Payouts that *read* the refusal record, turning an unanswered share of demands into a credit event a guarantee can pay on, are the trigger extension, and the latching and denominator rules that make them safe live with it ([Extensions](extensions.md)).

## 14. Privacy and disclosure

One principle governs identity: **it attaches where someone must be answerable for a promise, and holding needs no identified holder.** Signing a backing names a key with a public history, and binding it to a legal person is optional and priced like everything else. Pool presentment uses a fresh key for release and withdrawal, so it need not name a continuing identity; the payout channel may separately identify the counterparty. Holding and spending are net-zero and stay dark.
...
The dealers [§16](#16-what-emerges) grows are also a privacy service nobody has to build. Sell to a dealer unlinkably at bid, the dealer presents its acquired notes in bulk, and a retail holder never touches the lit end at all.
...
**The honest ceiling.** Privacy has increasing returns, and the cold start has none of it. This begins in a village, where the crowd to hide in is so small that everyone is identifiable, and no construction helps. At scale the floor is set by what must stay lit: identified issuance where a lender wants it, the settlement at final redemption, and the demand you publish when you exercise recourse. Identity disclosed through a payout channel is another limit. Everything between those points can be dark, and the design should be judged on whether it keeps it dark, not on the endpoints it deliberately lights.
