...
**The spent set is an accumulator with non-membership**, not a bare list: a reader must be able to establish that a nullifier is absent as of a commitment, which [§C2b.3](#c2b-failure-silence-and-recovery)'s snapshot redemption needs and a bare Merkle root over spent entries cannot give. A reader that replays the committed history holds the set and reads absence from it; a reader checking absence against the root alone needs the accumulator's proof. [The pool's recovery contract](pool-recovery.md) takes the first route and publishes no proof beside a release (C2b.3.3).
