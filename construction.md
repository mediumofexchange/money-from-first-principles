...
23. A commitment **commits to** the issuance log, the spent set, running totals and the standing demand record, plus the pending-lock set and the construction's supply objects: in the core, the note tree, the spent-set accumulator and the ordered statement log ([§C1.2](#c1-claims-and-wallets)); a profile declares its own. The void-by-spend check reads the spent set only. A commitment does not contain any of them, and anything checked against them has to be served ([§C0b](#c0b-the-payout-language-and-the-publication-layer)). **The spent set must support non-membership**, so that a reader can be shown a claim *not* spent as of a commitment **without replaying its history**, which a bare Merkle root over spent entries cannot do. A reader that does replay recomputes the set and reads absence from it, and every reader of [§C2b](#c2b-failure-silence-and-recovery)'s recovery path replays today, so no rule now in force spends this capability; it is kept because a compact absence certificate cannot be added later without a new construction version, and it is cheap in an accumulator chosen for it. **And the commitment binds the order of statements**, not only the resulting roots: two histories can reach one note-tree root while spending different notes, and a commitment that named only the root would leave the spent set ambiguous.
24. **One witnessed evaluation instant per presentation.** The instant is **named in the holder-authorized demand and agreed by the acceptance**, on one declared venue, no later than the latest witnessed index when the holder authorizes the demand. The claim layer defines that authorization: in the shielded pool the holding proof binds the notice, and the backer's acceptance signs its identity (pool recovery C3.2–C3.4); a profile using signed notices retains its signatures. Absent an acceptance, the demand's witnessed publication fixes the evaluation instant unless the claim layer's contract specifies otherwise. In the pool, readers retain the named instant and check C3.3's window at publication or earliest witnessing under service; publication does not replace it with the publication index. The instant is an index on one venue that every backing in the set declares; a set with no common venue cannot be presented in one demand, and presents per backing instead. Never a local timestamp chosen by the holder.
...
**Shared service per operator, construction domain and venue**, across the backings it serves in that scope. The anonymity set is the service scope rather than one backing's holders ([§14](money-from-first-principles.md#14-privacy-and-disclosure)), and a payment from several scoped backings is one shielded statement. [The authority and history contract](pool-authority.md) fixes how private spends remain authorized when backings replace their operators independently. The scope and its replacement links are public; a statement does not disclose which of those backings it spends.

**A note** is an opening *(construction domain, backing name, quantity, owner, randomness)*. Its **commitment** is the declared hash of the opening. Each service segment has an append-only **note tree** of its local outputs and imports only roots certified by finalized predecessor histories. The **owner** is the declared hash of a spend secret, and the receiver generates the secret (invariant 25): in v2 the payer receives only the owner value, builds the output note, and delivers the opening to the receiver privately. A successor adopting [the delivery and restoration contract](pool-delivery.md) instead has the receiver prepare the exact requested output and a seed-encrypted recovery capsule; its statements bind those capsules for public replication. That replaces the delivery procedure, not the note or ownership formula. **The nullifier** binds *(construction domain, commitment, spend secret)*, independently of operator, segment, anchor and path. Replacement never changes a note's commitment or nullifier.

**A spend** binds the construction domain, service segment and scope root, input anchors, nullifiers and output commitments. Its proof shows ownership and immutable nullifiers; note membership for positive inputs; private scope membership for every input and output backing, including padding; and bounded integer conservation separately for every hidden backing. Inputs may use different accepted roots from finalized imports or the current segment. The statement hides the spent leaves, backing names, quantities and owners; its public anchors disclose source history, not the leaf. Ordinary transfers require the holder's proof and the operator's receipt, never permission from a departed operator.

**A burn** has the same domain, segment, scope and ownership bindings, with its backing and quantity opened. **Redemption is a spend** whose output the backer owns: private like any transfer, and outstanding is unchanged by it (invariant 10). The backer burns what it takes in where its terms say so, in a separate public act, and only that lowers the count.

**Naming a note in public.** A demand ([§C3](#c3-presentation-and-dishonour)) names its claims by `H(nullifier)`: equality across demands is public at filing, the nullifier itself appears only at spend, and a duplicate over one note is visible. A non-service object ([§C2b.5](#c2b-failure-silence-and-recovery)) carries a membership proof for a real note without revealing its quantity. Both read the accepted-root forest and combined spent set the commitment binds.

**Admission** reads one view with record-derived scope and imports. It checks the pinned proof, scope and segment binding, accepted anchors, and nullifier/output uniqueness across the deduplicated imported history and local tail; issuance also requires K's signature and bounded supply. Nullifier insertion, output insertion, supply change and the receipt commit together or not at all (invariant 26). A forced scope lapse discards its incompatible unfinalized tail from active state; an elective change first witnesses live receipted work (C2.10.9). A restart in an unchanged current scope retains its durable tail (C2.8).

**Supply follows by induction from empty genesis and canonical finalized imports.** Only verified issuance introduces claims; spends conserve per backing and burns subtract. Shared ancestor events count once. Every finalized event involving a backing extends that backing's canonical predecessor; importing it through another backing neither creates a new event nor grants authority over it. Before serving a backing, the operator imports its complete canonical spentness. A stranger replays the supplied causal histories, checks whole-scope finality and recomputes roots and totals (invariants 12 and 23). Arbitrary root unions or replayable but unfinalized tails do not establish that induction. Currency remains the venue's ([§C2](#c2-sequencing)).
...
**E** separately names its original operator, venue and replacement rule, and the immutable **construction**, **version** and **configuration** fixing note domains, statement relations, hashes, proof system, circuit identities and bounds. Current authority follows the witnessed replacement chain; it is not a mutable field inside the immutable configuration. **Changing the construction or configuration is a new E, hence a new backing** (C1.6); replacing an operator under the existing E is not. [The authority contract](pool-authority.md) requires a new construction version, and [pool-v2.md](pool-v2.md) fixes that version bit for bit: the domain-only configuration, the scope, the segment, the statements, admission, the history, the receipt and finalized import. [pool-v1.md](pool-v1.md) preserves the historical fixed-operator bytes and circuits; they do not implement replacement and must not be reinterpreted as doing so.
...
| Backer, at redemption | the presenter key, the specific claims, the quantity; any identity disclosed through the payout channel | how the claims travelled |
...
**Aborted presentations.** [§C3](#c3-presentation-and-dishonour)'s pool demand publishes the notes' tags beside a proof-authorized notice and reserves the notes without publishing their nullifiers. Each tag links to its note's eventual spend, so an aborted note must be spent to a fresh one before reuse.
...
### C2.10 Shared private histories across replacement

[The authority and history contract](pool-authority.md) defines C2.10.1–9:
public service scopes with private membership proofs, whole-scope finality,
exact per-backing predecessors, deduplicated finalized imports, immutable
nullifiers, and scope-bound receipts. These requirements generalize C2.4 and
C2.7 for opaque histories. A late checkpoint lapses for its entire scope;
the authenticated scope and replacement evidence prove that step in C2.7's
descent, whereas missing scope data proves nothing. A checkpoint whose
committed evidence fails the rules is excluded and passed on that evidence
([C2.10.10–12](pool-fault.md)); a checkpoint the reader cannot classify blocks.
The costs include shared
tail finality and transitive data availability; no common replacement
authority is introduced. A scope never spans independent venue clocks.
...
**C2b.3 When sequencers go dark**, claims go illiquid rather than dead. Value discounts until they return, and after the declared silence ([§C2b.6](#c2b-failure-silence-and-recovery)) redemption against the last witnessed snapshot opens without co-signature, the claim shown unspent as of that snapshot against its spent set (invariant 23). **The snapshot is the backing's rather than a key's**: the last commitment, witnessed from a party that was in force for this backing when it published it, whose directory carries the backing. A handover does not discard it, so a successor that has committed nothing is redeemed against the backing's last, whichever term of the chain it fell in ([§C2.7.1](#c27-taking-over)). Read instead against whoever is in force now, the remedy would vanish the moment a rule-holder named anybody at all, and the rule-holder is the backer by default, which is the party that owes the money.

**C2b.3a Shielded recovery preserves finalized holdings.** A holder proves a positive quantity, authority over the note, inclusion in the canonical finalized history carried by the snapshot, and the note's immutable nullifier absent from its spent set. Eligibility also accounts for valid recovery settlements witnessed since that snapshot: the same note cannot settle twice. Absence of an arbitrary nullifier is not proof of a holding. An unwitnessed spend or receipt neither creates a recoverable holding nor redirects another holder's redemption. Recovery does not finalize the discarded tail. A statement already finalized remains final through silence and replacement; a recipient who accepted an unwitnessed payment retains evidence of the operator's acceptance, not authority over the payer's snapshot holding or a guarantee of compensation.

**C2b.3b Snapshot redemption preserves supply and consent.** After the backer's acceptance, the holder's release publishes the consumed nullifiers and commits to the backer-owned replacement notes as one recovery settlement at the declared witness venue. It settles the offered set under §C3 and invariant 27; publication of bare nullifiers is not a settlement. Redemption transfers claims to the backer and leaves outstanding unchanged (invariants 10 and 12); a burn requires its own authorization. Returning service adopts the valid witnessed settlement's effects, including its consumed nullifiers and replacement notes, before new service under C2b.4. These are requirements on a construction that supports recovery, not new statement kinds for a construction that does not.

**C2b.3c No spend-based redirection window in the shielded core.** An unwitnessed spend is not a challenge that pays its outputs instead of the snapshot holder. The former named-payee challenge and its window belong only to [the transparent profile](extensions.md#the-transparent-profile). Finalized-state proofs and the witnessed recovery record, rather than a wait for unwitnessed transfers to surface, decide a shielded recovery holding. This removes only that challenge window, not the declared silence duration or §C3's settlement conditions and timeouts.

Serving a claim the venue shows consumed by a valid recovery settlement is equivocation, provable against its own commitment (invariant 22). The holding proof runs against the published trail, which replicas serve because publication was the point. Where nobody replicated, the claim waits, and the availability price was already on the table. Whether a sequencer can be replaced at all is answered in **E**. The construction must specify the recovery proofs, settlements and return adoption together before claiming this remedy; [pool-v2 §7.4](pool-v2.md#74-redemption-and-what-this-version-does-not-carry) explicitly does not supply them. [The presentation and recovery contract](pool-recovery.md) specifies them: the snapshot and the no-commitment clock (C2b.3.1, C2b.6.1), venue publications and their force (C2b.3.2–3), the return as a new segment with its adopted block (C2b.4.1–2), and the non-service request and count (C2b.5.1–2). A construction version instantiating that contract is still required before the remedy is claimed.

**C2b.4 Returning from silence is committing.** A sequencer returning from silence adopts every nullifier witnessed during the gap before co-signing again. In the shielded construction these are the nullifiers of valid recovery settlements, whose replacement notes are adopted with them (C2b.3b); bare nullifier publications have no such force. The gap runs until a commitment is witnessed and a publication inside it has force, so the sequencer rebuilds from its last commitment — what it co-signed after that commitment was never witnessed, and finality means witnessed — adopts the gap, commits, and serves from the index after, a publication at the commitment's own index being judged against the record strictly before it.
...
**Presentment is committed, and published when it matters.** The holder authorizes a notice naming the backing, the quantity, the specific claims by `H(nullifier)`, the evaluation instant and a deadline of their choosing. In the shielded pool the holding proof binds that notice, including its fresh presenter key; no signature is added to the demand (pool recovery C3.2–C3.3a). The presenter key authorizes release and withdrawal, not a claim about the demander's identity. The named instant is an offer an acceptance may agree; without one, publication establishes the evidence under invariant 24. Both times are witnessed indices ([§C0b](#c0b-the-payout-language-and-the-publication-layer)). Publishing turns a private commitment into evidence, done when the backer is not cooperating.
...
**What this does not achieve.** In a chain asset the exchange can be atomic, and refusal becomes proof. In kind it cannot, since each party may assert what suits them, so a demand and its silence is evidence rather than proof of non-payment, and a dishonest holder can damage a name cheaply. The holding proof requires real notes and binds the notice; it does not identify a person, prove that the presenter key participated in making the demand, or make an allegation about external payment true. A copied demand does not identify its publisher as the holder. Even a signature under a fresh key would not establish a civil identity or a continuing reputation. Any attribution supported by separate evidence is outside the pool's validity rules; the core supplies no identity-based restraint on this abuse.

Over the shielded pool, [the presentation and recovery contract](pool-recovery.md) defines C3.1–C3.8: the tag, the holding proof, the demand, the acceptance, the settlement statement, the release and the withdrawal, the standing-demand record and pending-lock set inside the segment's history, and deadlines at the door. It covers an empty reliance set and a payout settling outside the claim layer, and a construction version carrying those objects is required before a pool backing presents.
...
**Redirecting shielded snapshot redemption through an unwitnessed spend.** The inherited challenge named one payee; a private spend commits to two outputs and may merge two inputs. Redirecting a whole input to one output misallocates change, while applying whole statements needs conflict ordering and their unfinalized dependencies. C2b.3a–c retires that redirection and its challenge window from the shielded core, preserving finalized holdings and valid witnessed recovery settlements instead. The cost is explicit: a recipient who accepted an unwitnessed payment cannot take value from the payer's snapshot holding through that payment. The transparent profile retains its existing, limited challenge behavior.

**An invalid commitment resets the no-commitment clock.** A stream of signed
bad proofs kept redemption shut forever, while even a successor faced blocked
descent. Authenticated exclusion (C2.10.10–12) passes the checkpoint to the last
valid state without erasing the fault; unavailable evidence still blocks.

**A commitment carrying nothing for a backing closes its interval.** This
left a dropped backing only the non-service count and made its clock depend
on unrelated scopes' classification. C2b.6.1 now reads the backing's snapshot:
the declared duration prices a drop as darkness, and the clock requires the
same evidence as that snapshot.
