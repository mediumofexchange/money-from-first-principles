...
A venue is named together with its **finality rule**, the depth or gadget under which an index counts as witnessed there. That is a floor under the interval, or two sequencers answer [§C3](#c3-presentation-and-dishonour)'s release predicate differently. It also fixes the venue's **lag**: the least number of indices by which an act a party signs is witnessed after the clock it signs at — none where publication lands at the clock's own index, the depth plus one where a chain includes in its next block and reads behind itself by that depth — which the replacement rule below floors a handover's lead on, and which the venue's name must therefore fix, as it fixes the depth: two readers of one name with two lags would disagree forever about who was in force at a past index. Witnessing pins order rather than validity (invariant 22 stops divergent histories): backers replicate what exonerates them, holders what pays them. One addition is worth the bytes, which is the backer's standing bid on its own claims, signed, sized, dated ([§14](money-from-first-principles.md#14-privacy-and-disclosure)).
