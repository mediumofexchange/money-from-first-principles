...
**The mechanism.** A per-backing public ledger of key-controlled balances. A holder transfers by signature at its next nonce, the operator co-signs each operation into the backing's log, and commitments carry the log, the balances and the standing demands. Double-spend prevention is free, `outstanding = issued − burned` is exact from the public log, and the issuance statement names the recipient. Where the core reads a nullifier this profile reads the holder's own signed spend at its sequence position: the non-service object is a signed transfer request checked against the last committed balance state; a demand's commitment rests on the lock alone; snapshot redemption publishes the holder's signed spend at the venue in place of a nullifier.

**The profile's challenge window.** A snapshot redemption stands for its declared challenge window. During it, anyone may publish the holder-signed spend consuming the claimed sequence position; a valid challenge redirects redemption to the payee that spend names. The challenge must connect to the claimant's position in the pre-gap snapshot. A claimant can move its own sequence before claiming, so this binds a careless double-spender, never a deliberate one. It does not rescue an arbitrary uncommitted history. This is the transparent profile's existing remedy, not the shielded core's: Construction C2b.3a–c preserves finalized shielded holdings without spend-based redirection.
