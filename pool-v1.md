...
**A proof** for a key is its 256 siblings, the sibling at height `h` being the other child of the path node's parent at height `h + 1`, sent as 32 bytes of **map** followed by the siblings not omitted. The map is a 256-bit value whose bit `h`, numbered as every bit in this document is (bit `0` the most significant), is `1` exactly when the sibling at height `h` equals `e_h`; those siblings are omitted and every other sibling is sent in ascending height. A proof whose map is clear for a sibling equal to `e_h` is malformed, so one set of siblings has one encoding; a map set at a height where the true sibling is not `e_h` carries the path to some other root, which a verifier refuses as it refuses any wrong sibling. **Non-membership** of `nf` at `spentRoot` is a proof whose path carries `e_0` at key `nf` to `spentRoot`; **membership** is a proof carrying `leaf(nf)` there. Construction §C2b.3's snapshot redemption was expected to read the holder's non-membership proof at the last witnessed commitment's `spentRoot`; [the recovery contract](pool-recovery.md) since has every reader of force read absence from the set it replays instead (C2b.3.3). This version is historical and is not reinterpreted; the venue-side record redemption also needs is v2's ([§5.4](#54-redemption-and-what-this-version-does-not-carry)).
