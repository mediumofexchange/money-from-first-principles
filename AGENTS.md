...
| `pool-fault.md` | **proposed fault contract** | Proposal, not adopted: how a checkpoint whose committed evidence fails the rules is classified and passed by descent, snapshot, count, clock and receipts; the revised clock; the amendments its adoption applies. The maintainer selects. |
